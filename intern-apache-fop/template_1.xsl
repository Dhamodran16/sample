...
<xsl:param name="dmDmCodeStringVar" select="'DMC CODE'"/>
  ...
  <!-- Global flag: is this data module marked as update-highlighted? -->
  <xsl:variable name="isUpdateHighlighted"
                select="/dmodule/identAndStatusSection/dmStatus/reasonForUpdate/@updateHighlight = '1'"/>
...
<xsl:number level="multiple" count="proceduralStep" format="1.1.1.1"/>
  ...
  <xsl:template match="para|p|text()">
    <fo:block>
      <!-- When the data module is update-highlighted, draw a change bar at the page's left margin.
           The negative margin moves the border into the outer page margin while padding
           keeps the text aligned with unmarked content. -->
      <xsl:if test="$isUpdateHighlighted">
        <xsl:attribute name="margin-left">-5mm</xsl:attribute>
        <xsl:attribute name="padding-left">5mm</xsl:attribute>
        <xsl:attribute name="border-left">0.75pt solid #000000</xsl:attribute>
      </xsl:if>
